...
Here's a look at some Elm code:
...
This is an implementation of a linked list and a ~length~ function for it. (This is just an example; Elm already has a built-in linked list type.)

Let's break this down a little bit:

- ~type IntList~: defines a new *type*. In an object-oriented language like JavaScript or Python, we would typically define classes, but in a functional language, instead you define types. Built-in types include ~Int~, ~String~, and many others. Types have names which always begin with capital letters.

- ~Nil~: the things after a type are the different variants. The first variant of the ~IntList~ type is called ~Nil~. This means that you can use ~Nil~ anywhere and it is a valid ~IntList~. Like types, variants must also have names that begin with a capital letter.

- ~Cons Int IntList~: defines the other variant of ~IntList~ which is called ~Cons~. Unlike ~Nil~, ~Cons~ is not standalone. ~Cons~ contains an ~Int~ and a ~IntList~. ~Cons~ (and also ~Nil~, but it's more obvious here) is a *constructor*. Like constructors in object-oriented languages, constructors are functions that let us "produce" a member of a type. (In this example, if we call ~Cons~ with an ~Int~ and an ~IntList~, we will have a ~IntList~.) However, unlike constructors in object-oriented languages, which are responsible for setting up initial state of an object, here the constructor is almost inert.

- ~{-| .. -}~: this is a documentation comment which can serve to describe the thing following it (or sometimes, the whole module).

- ~length : IntList -> Int~. This is a *type signature*, which tells us the type of something. All top-level things in Elm must have a type signature. This type signature says "~length~ has the type (~:~) of a function (~->~) which takes ~IntList~ and produces ~Int~". Type signatures are always of the form ~thing : type~. The ~->~ arrow is how we describe functions. More about this later.

- ~length l =~: begins the function definition. In Elm, function definitions look like mathematical equations. ~length~ is a function of one argument, so we see an argument before the equals sign, but you use the same syntax to define things that don't have arguments (or sometimes even for some that do). Note that ~length~ has a name that starts with a lowercase letter because it isn't a type.

- ~case l of~. This is called pattern matching. This syntax says "check if ~l~ is any of these variants". It's kind of like a giant if-elseif-else statement, or a switch statement (in languages that have them).

- ~Nil ->~: if ~l~ is ~Nil~, then we follow this branch and produce the value 0.

- ~Cons _ remainder ->~: on the other hand, if we have a ~Cons~, we want to do something different. In particular, we will want to use the ~IntList~ that was "contained" in the ~Cons~. By using pattern matching, we not only get to check what variant we have, but get to extract its data. This is the other side of the "constructor" coin in Elm -- not only is it used to create values, but also to deconstruct them.

- ~-- This is the recursive case.~. Elm supports single-line comments beginning with ~--~ and multi-line comments beginning with ~{-~ and ending with ~-}~. (Documentation comments, above, start with ~{-|~, with the ~|~ indicating "the thing below".)

- ~length remainder + 1~: this is the recursive application of the function. In Elm, to call a function, you just write the name of the function followed by any arguments. Function application "binds tightly", so ~length remainder + 1~ means ~(length remainder) + 1~ and not ~length (remainder + 1)~.

When you look at Elm code, you will notice first that the syntax is very different from other languages you've seen. (Instead of the C family languages, which inspired JavaScript syntax, Elm is more inspired by the ML family of languages, which includes Haskell.) But apart from this surface difference, there are two important structural differences that make Elm so different:

- Elm is *statically typed*. This means that every variable has a type that can be determined before your code even runs. (Whereas in a dynamically-typed language, the contents of a variable are not really known until runtime.) Before your Elm code can run, it will be *typechecked*, which means each operation is examined to see whether it makes sense with the types that we have.

  Statically typed languages have the advantage that they can prevent a lot of the goofy stuff that happens in JavaScript. In JavaScript, if you try to add a number to an object, you get ~"[object Object]5"~. In Elm, you just can't do this; if you try, the compiler will yell at you and you won't be able to run your code. This also prevents stuff like ~undefined is not a function~ or ~could not access property foo of null~. Static typing has the *disadvantage* that you have to spend more time thinking about your program before you write any of it. You will have to figure out what types you need and how they fit together. This effort usually leads to a better program, but it can be frustrating and feel "slower" than just writing JavaScript.

- Elm is a *pure* language, which means that no operation can have a side effect.[*] This means that when you call a function, the only thing it can do is return a value. This also means that you can't change a variable (doing so is, by definition, a side effect). It *also* means that if you call a function with the same arguments, you *must* get the same results. The advantage here is that no data is hidden and no code can change a variable somewhere without you knowing about it. The disadvantage is that a lot of things tend to rely on some kind of implicit state, and in Elm you have to always make that state explicit.
...
You'll note that there's no ~return~ statement in Elm. There aren't really even statements at all! No operation can have a side effect, so every operation must have a value, which means the language only has expressions. Even ~if~ statements have values -- you could do ~xIsBig = if x > 5 then True else False~.

There's also no real "loop" operations -- no ~for~, no ~while~. Instead, you have to write your own recursive functions (or use helpers like ~fold~).
...
Now, ~MyList a~ is the type of a list that holds ~a~ elements. ~MyList Int~ is the type of lists that hold ~Ints~, etc. ~MyList~ by itself is incomplete -- you need to provide another type for it to "work".

This idea is pretty funky and you may stumble over it the first few times you use it.
*** Some useful types
Here are some examples of this kind of generic type. First, we've already seen that lists are generic -- the list type in Elm is called ~List~.

There's a type called ~Maybe~. It's defined like this:
...
N.B. It's possible to overuse these. If you find yourself using a type with ~Maybe~ s nested in ~Result~ s or the other way around, that's a good clue that you might want to define a new type. It's also possible to lean on ~Maybe~ as a crutch instead of correctly modeling when you will have (or not have) the related value. More about this later.
*** Partial function application
Here is how you define a function of more than one argument in Elm:
...
(This function is completely useless, because you could just use ~Cons~ directly, but we'll use it to demonstrate some stuff about syntax.)

The syntax for type signatures with multiple arguments may look a little strange: ~Int -> IntList -> IntList~. It almost looks like, instead of one function of two arguments, there's two functions of one argument! And the reason for this is that actually, it is. In Elm (and many related languages) there's the concept of "partial function application", which means you can provide e.g. only one of the two arguments, and in so doing, create a new function. So although you could do this:

#+BEGIN_SRC elm
  push5 : IntList -> IntList
  push5 l =
      push 5 l
#+END_SRC
...
Sometimes this is also called "currying" (although technically, currying is something else; everyone just uses the word wrong). So in a real sense, ~push~ is a function that takes one argument, and which returns a function that takes the second argument. Elm does this for us automatically.
*** More operators
...
- Tuples
*** Exercises
- Install Elm on your computer. Make sure you know how to build and run Elm programs.

- https://ellie-app.com/5v5GwS5rKFra1 is a "problem set". You can solve it on Ellie or on your own computer, as you prefer.

FIXME: maybe don't jump into rewriting ~map~. Instead spend more time defining types and constructors and pattern matching them.
...
- Need to emphasize the utility of constantly asking "What is the type of this thing?" No way to ask Elm to tell us the type unfortunately.
